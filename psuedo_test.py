...
# A novel method of creating groups to split strings into
	...
	for idx in range(n-num_groups):
		...
		for i in range(len(strings)):
			if not used(groups, strings[i]):
				correlations = [correlation(groups[x], strings[i], ns_groups[x], ns_strings[i]) for x in range(len(groups))]
				correlations.sort()
		...
		if max(corr_diffs) != 0:
			...
			ns_groups[correlations.index(max(correlations))] = sum(count_nodes(groups[correlations.index(max(correlations))]))
		else:
			for i in range(len(strings)):
				if not used(groups, strings[i]):
